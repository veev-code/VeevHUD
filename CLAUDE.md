...
- `BuffRemindersClickOverlay.lua` — Opt-in (default OFF) out-of-combat click-to-cast for buff reminders. Invisible per-alert `SecureActionButton` hit zones in an isolated UIParent-anchored subtree, synced to the visual icons only out of combat (tracking slide animations per-frame), hidden in combat by a secure `[combat] hide; show` state driver. Actions are resolved OOC by `BuffReminders:GetClickAction` into static `type="spell"` self-casts or `type="macro"` conditional-target/item/weapon-hand macros (macro conditionals are the sanctioned per-click decision channel). **The file header documents the WoW secure-frame constraint model (protection propagation, combat lockdown, no aura conditionals) and the five design rules — read it before touching anything secure, and never parent/anchor secure frames into the HUD tree (caused the v1.0.202 ADDON_ACTION_BLOCKED incident).**
